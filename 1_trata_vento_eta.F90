...
!  integer, parameter :: dt = 3600
  ...
  character(len=100) :: ncfile, linha,dimname,uwind,vwind,xlon,ylat,ttime
  real(dp), allocatable :: u_interp(:), v_interp(:)
  namelist /atmosferico/ ncfile,dt,xlon,ylat,ttime,uwind,vwind

  open(unit=10, file="namelist_pre.nml", status="old")
  read(10, nml=atmosferico)
  close(10)
  ! Arquivos de entrada
!  ncfile = '../dados/Eta03_BESM_2007010100_2D.nc'
...
!  call check(nf90_inq_dimid(ncid, lat_varid, dimid))
  ...
  ! === Converter UTM -> Geográfica (precisa escrever função própria ou usar lib) ===
!  print*,"CONVERTENDO COORDENADAS UTM PARA LAT/LON"
!  do i = 1, nnos
!    call utm2geo(xutm(i), yutm(i), lon_malha(i), lat_malha(i))
!  end do
...
contains
 ...
 !   subroutine utm2geo(x, y, lon, lat)
 !   real(dp), intent(in) :: x, y
 !   real(dp), intent(out) :: lon, lat
 !   ! Aqui você deve usar uma rotina real de conversão UTM -> lat/lon
 !   ! Placeholder:
 !   lon = x / 100000.0 - 60.0
 !   lat = y / 100000.0 - 10.0
 ! end subroutine
